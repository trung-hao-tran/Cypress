...
const QuillEditor: React.FC<QuillEditorProps> = ({
  ...
  useEffect(() => {
    ...
    const subscription = room
      .on('presence', { event: 'sync' }, () => {
        ...
        // Deduplicate collaborators by id to avoid React key conflicts
        const uniqueCollaborators = Array.from(
          new Map(newCollaborators.map((c: any) => [c.id, c])).values()
  ...
  return (
    <>
      ...
      <ScrollArea type="always" className="h-[calc(100vh-80px)]">
        <div
          className="flex
          justify-center
          items-center
